...
一个完整的电商系统，采用现阶段流行技术来实现，采用前后端分离继续编写。基于尚硅谷商城项目，用于学习相关技术。
...
电商项目，包括前台商城系统以及后台管理系统，基于 SpringCloud + SpringCloudAlibaba + MyBatis-Plus实现，采用 Docker 容器化部署。前台商城系统包括：用户登录、注册、商品搜索、商品详情、购物车、下订单流程、秒杀活动等模块。后台管理系统包括：系统管理、商品系统、优惠营销、库存系统、订单系统、用户系统、内容管理等七大模块。
...
```
legmall
├── legmall-common -- 工具类及通用代码
├── renren-generator -- 人人开源项目的代码生成器
├── legmall-auth-server -- 认证中心（社交登录、OAuth2.0、单点登录）
├── legmall-cart -- 购物车服务
├── legmall-coupon -- 优惠卷服务
├── legmall-gateway -- 统一配置网关
├── legmall-order -- 订单服务
├── legmall-product -- 商品服务
├── legmall-search -- 检索服务
├── legmall-seckill -- 秒杀服务
├── legmall-third-party -- 第三方服务
├── legmall-ware -- 仓储服务
└── legmall-member -- 会员服务
...
```
192.168.77.130	legmall.com
192.168.77.130	search.legmall.com
192.168.77.130  item.legmall.com
192.168.77.130  auth.legmall.com
192.168.77.130  cart.legmall.com
192.168.77.130  order.legmall.com
192.168.77.130  member.legmall.com
192.168.77.130  seckill.legmall.com
以上端口换成自己Linux的ip地址
```
...
2、legmall.conf中添加如下配置
server {
    listen       80;
    server_name  legmall.com  *.legmall.com hjl.mynatapp.cc;
    ...
    #支付异步回调的一个配置
    location /payed/ {
        proxy_set_header Host order.legmall.com;        #不让请求头丢失
        proxy_pass http://legmall;
    }
...
- 克隆整个后端项目 `legmall` ，并导入 IDEA 中完成编译
